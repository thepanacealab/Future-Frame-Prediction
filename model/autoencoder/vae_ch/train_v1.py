...
x = tf.layers.conv2d_transpose(x, filters=128, kernel_size=2, strides=1, padding='same', activation=tf.nn.relu)
#print(x.shape)
x = tf.nn.dropout(x, keep_prob)

x = tf.layers.conv2d_transpose(x, filters=64, kernel_size=2, strides=2, padding='same', activation=tf.nn.relu)
#print(x.shape)
x = tf.nn.dropout(x, keep_prob)
x = tf.layers.conv2d_transpose(x, filters=32, kernel_size=2, strides=2, padding='same', activation=tf.nn.relu)
#print(x.shape)
